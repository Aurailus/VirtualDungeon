...
export default class AssetUploader {
	...
	private async uploadFile(file: UploadFile): Promise<void> {
		return new Promise((resolve) => {
			...
			xhr.addEventListener('load', () => {
				file.status = parseInt(xhr.responseText, 10);
				this.renderFileList();
				resolve();
			});
	...
	private renderFileList() {
		...
		let allValid = this.filesList.length !== 0;
		...
		'<div class=\'input_wrap\'><input name=\'identifier\' maxlength=32 spellcheck=\'false\'/></div>' :
