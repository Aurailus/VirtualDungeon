...
import { TileItem as HistoryTile } from '../history/HistoryType';
...
export default class ArchitectMode extends Mode {
	...
	update(cursorPos: Vec2, input: InputManager) {
		...
		if (input.mouseDown()) {
			...
			if (this.drawn.length) {
				this.history.push({ type: 'tile', items: this.drawn });
				this.drawn = [];
			}
		}
	}
	...
	private drawTile(pos: Vec2, solid: boolean) {
		if (!this.map.activeLayer) {
			console.warn('Tried to draw tile without an active layer.');
			return;
		}

		let tile = solid ? this.activeTileset : 0;
		const lastTile = this.map.activeLayer.getTile(this.activeLayer, pos);

		if (this.map.activeLayer.setTile(this.activeLayer, tile, pos)) {
			this.drawn.push({
				pos, tile: { pre: lastTile, post: tile },
				layer: this.activeLayer, mapLayer: this.map.activeLayer.index
			});
		}
	}
}
